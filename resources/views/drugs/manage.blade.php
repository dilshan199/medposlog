...
@section('title', 'Medposlog | Manage Drugs')
...
@section('content')
    ...
    <div class="w-full">
        <div class="flex items-center justify-between">
            <ul class="flex text-sm text-gray-600 font-normal space-x-1">
                ...
                <li>
                    <span class="font-medium text-azure-radiance-500">Manage Drugs</span>
                </li>
            </ul>
            @if (session('success'))
    ...
    <div class="w-full rounded-md shadow-md px-3 py-3 bg-white mt-3">
        <form action="{{ route('drugs.store') }}" method="post">
            ...
            <div class="grid grid-cols-4 gap-2">
                <div>
                    <label for="code" class="w-full font-medium text-sm text-gray-600">Short Code <span class="text-red-600">*</span></label>
                    <div>
                        <select name="code" id="code" class="p-2 rounded-md border-gray-300 w-full">
                            <option value="" disabled selected>~~ Select Code ~~</option>
                            <option value="ND">Normal Drugs</option>
                            <option value="SD">Special Drugs</option>
                            <option value="IN">Insulin</option>
                        </select>
                        <span class="text-xs font-medium text-red-600">{{ $errors->first('code') }}</span>
                    </div>
                </div>
                <div class="">
                    <label for="drug_name" class="w-full font-medium text-sm text-gray-600">Drugs<span class="text-red-600">*</span></label>
                    <div>
                        <input type="text" name="drug_name" id="drug_name" class="p-2 rounded-md border-gray-300 w-full" autocomplete="off" value="">
                        <span class="text-xs font-medium text-red-600">{{ $errors->first('drug_name') }}</span>
                    </div>
                </div>
                <div>
                    <label for="dosage" class="w-full font-medium text-sm text-gray-600">Dosage <span class="text-red-600">*</span></label>
                    <div>
                        <input type="text" name="dosage" id="dosage" class="p-2 rounded-md border-gray-300 w-full" autocomplete="off" value="">
                        <span class="text-xs font-medium text-red-600">{{ $errors->first('dosage') }}</span>
                    </div>
                </div>
                <div>
                    <button type="submit" name="submit" value="Create User" class="p-2 rounded-md bg-azure-radiance-500 hover:bg-azure-radiance-300 text-white font-medium mt-6">Add Drugs</button>
                </div>
            </div>
            <div class="mt-3">
            ...
            @if (count($drugs) > 0)
                <table class="w-full border border-gray-300 text-sm">
                    <thead>
                        ...
                        <th class="border border-gray-300">Drugs</th>
                        <th class="border border-gray-300">Dosage</th>
                        <th class="border border-gray-300">Action</th>
                    </thead>
                    <tbody>
                        @foreach ($drugs as $dru)
                            <tr>
                                <td class="border border-gray-300 py-1 px-1">{{ $dru->drug_id }}</td>
                                <td class="border border-gray-300 py-1 px-1">{{ $dru->code }}</td>
                                <td class="border border-gray-300 py-1 px-1">{{ $dru->drug_name }}</td>
                                <td class="border border-gray-300 py-1 px-1">{{ $dru->dosage }}</td>
                                <td class="border border-gray-300 py-1 px-1" width="25%">
                                    <ul class="flex item-center space-x-3 justify-center">
                                        <li>
                                            <a type="button" id="editButton" data-url="{{ route('drugs.edit', $dru->drug_id )}}" class="text-gray-400 hover:text-azure-radiance-500 cursor-pointer"><i class="bi bi-pencil-square"></i>&nbsp;Edit</a>
                                        </li>
                                        <li>
                                            <a type="button" id="{{ $dru->drug_id }}" class="text-gray-400 hover:text-azure-radiance-500 cursor-pointer remove"><i class="bi bi-trash"></i>&nbsp;Delete</a>
                                        </li>
                                    </ul>
                                </td>
                            </tr>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-24">
                ...
                <form action="" method="post">
                    <div class="px-6 py-2 space-y-6">
                        <input type="hidden" name="drug_id" id="drug_id_2" value="">
                        <input type="hidden" name="_token" id="_token" value="{{ csrf_token() }}">
                        <div class="w-full mb-3">
                            <label for="code" class="text-sm font-medium w-full">Short Code</label>
                            <select name="code" id="code_2" class="p-2 rounded-md border-gray-300 w-full">
                        ...
                        <div class="w-full mb-3">
                            <label for="drug_name" class="text-sm font-medium w-full">Drug</label>
                            <input type="text" name="drug_name" id="drug_name_2" class="p-1 rounded-md w-full" value="">
                        </div>
                        <div class="w-full mb-3">
                            <label for="dosage" class="text-sm font-medium w-full">Dosage</label>
                            <input type="text" name="dosage" id="dosage_2" class="p-1 rounded-md w-full" value="">
                        </div>
                    </div>
                    <!-- Modal footer -->
    ...
    <script src="{{ asset('js/edit-drugs.js') }}" type="text/javascript"></script>
@endsection
