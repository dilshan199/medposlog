...
@section('title', 'MedOne | Manage Frequency')
...
@section('content')
    ...
    <div class="w-full">
        <div class="flex items-center justify-between">
            <ul class="flex text-sm text-gray-600 font-normal space-x-1">
                ...
                <li>
                    <span class="font-medium text-azure-radiance-500">Manage Frequency</span>
                </li>
            </ul>
            @if (session('success'))
    ...
    <div class="w-full rounded-md shadow-md px-3 py-3 bg-white mt-3">
        <form action="{{ route('frequency.store') }}" method="post">
            ...
            <div class="grid grid-cols-3 gap-2">
                <div class="">
                    <label for="frequency" class="w-full font-medium text-sm text-gray-600">Frequency<span class="text-red-600">*</span></label>
                    <div>
                        <input type="text" name="frequency" id="frequency" class="p-2 rounded-md border-gray-300 w-full" autocomplete="off" value="">
                        <span class="text-xs font-medium text-red-600">{{ $errors->first('frequency') }}</span>
                    </div>
                </div>
                <div>
                    <button type="submit" name="submit" value="Create User" class="p-2 rounded-md bg-azure-radiance-500 hover:bg-azure-radiance-300 text-white font-medium mt-6">Add Frequency</button>
                </div>
                <div></div>
            </div>
            <div class="mt-3">
            ...
            @if (count($frequency) > 0)
                <table class="w-full border border-gray-300 text-sm">
                    <thead>
                        ...
                        <th class="border border-gray-300">Frequency</th>
                        <th class="border border-gray-300">Action</th>
                    </thead>
                    <tbody>
                        @foreach ($frequency as $fre)
                            <tr>
                                <td class="border border-gray-300 py-1 px-1">{{ $fre->frequency_id }}</td>
                                <td class="border border-gray-300 py-1 px-1">{{ $fre->frequency }}</td>
                                <td class="border border-gray-300 py-1 px-1" width="25%">
                                    <ul class="flex item-center space-x-3 justify-center">
                                        <li>
                                            <a type="button" id="editButton" data-url="{{ route('frequency.edit', $fre->frequency_id )}}" class="text-gray-400 hover:text-azure-radiance-500 cursor-pointer"><i class="bi bi-pencil-square"></i>&nbsp;Edit</a>
                                        </li>
                                        <li>
                                            <a type="button" id="{{ $fre->frequency_id }}" class="text-gray-400 hover:text-azure-radiance-500 cursor-pointer remove"><i class="bi bi-trash"></i>&nbsp;Delete</a>
                                        </li>
                                    </ul>
                                </td>
                            </tr>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-24">
                ...
                <form action="" method="post">
                    <div class="px-6 py-2 space-y-6">
                        <input type="hidden" name="frequency_id" id="frequency_id_2" value="">
                        <input type="hidden" name="_token" id="_token" value="{{ csrf_token() }}">
                        <div class="w-full mb-3">
                            <label for="problem" class="text-sm font-medium w-full">Problem</label>
                            <input type="text" name="frequency" id="frequency_2" class="p-1 rounded-md w-full" value="">
                        </div>
                    </div>
                    <!-- Modal footer -->
    ...
    <script src="{{ asset('js/edit-frequency.js') }}" type="text/javascript"></script>
@endsection
