...
@section('content')
            ...
            @if (count($clinic) > 0)
                <table class="w-full border border-gray-300 text-sm">
                    <thead>
                        ...
                        <th class="border border-gray-300">Clinic</th>
                        <th class="border border-gray-300">Action</th>
                    </thead>
                    <tbody>
                        @foreach ($clinic as $cli)
                            <tr>
                                <td class="border border-gray-300 py-1 px-1">{{ $cli->clinic_id }}</td>
                                <td class="border border-gray-300 py-1 px-1">{{ $cli->clinic_followup }}</td>
                                <td class="border border-gray-300 py-1 px-1" width="25%">
                                    <ul class="flex item-center space-x-3 justify-center">
                                        <li>
                                            <a type="button" id="editButton" data-url="{{ route('clinic.edit', $cli->clinic_id) }}" class="text-gray-400 hover:text-azure-radiance-500 cursor-pointer"><i class="bi bi-pencil-square"></i>&nbsp;Edit</a>
                                        </li>
                                        <li>
                                            <a type="button" id="{{ $cli->clinic_id }}" class="text-gray-400 hover:text-azure-radiance-500 cursor-pointer remove"><i class="bi bi-trash"></i>&nbsp;Delete</a>
                                        </li>
                                    </ul>
                                </td>
                            </tr>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-24">
                ...
                <form action="" method="post">
                    <div class="px-6 py-2 space-y-6">
                        <input type="hidden" name="clinic_id" id="clinic_id_2" value="">
                        <input type="hidden" name="_token" id="_token" value="{{ csrf_token() }}">
                        <div class="w-full mb-3">
                            <label for="clinic" class="text-sm font-medium w-full">Clinic</label>
                            <input type="text" name="clinic" id="clinic_2" class="p-1 rounded-md w-full" value="">
                        </div>
                    </div>
                    <!-- Modal footer -->
    ...
    <script src="{{ asset('js/edit-clinic.js') }}" type="text/javascript"></script>
@endsection
