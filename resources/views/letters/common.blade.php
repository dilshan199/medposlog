...
@section('content')
    ...
    <div class="w-full">
        <div class="flex items-center justify-between">
            <ul class="flex text-sm text-gray-600 font-normal space-x-1">
                ...
                <li>
                    <span class="font-medium text-azure-radiance-500">Common Letter</span>
                </li>
            </ul>
        </div>
    </div>
    ...
    <div class="bg-white w-[793.70px] h-[1118.74px] shadow-sm m-auto p-[37.79px]">
        <form action="{{ route('letters.common-print') }}" method="post">
            @csrf
            ...
            <div class="mt-[50px] w-full" style="line-height: 4">
                ...
                <p><input type="text" name="c_whom" id="c_whom" class="p-1 rounded-md border border-gray-400" value="@php if(session('c_whom')){echo session('c_whom');}else{echo '';} @endphp"></p>
            </div>
            <div class="flex items-center space-x-3 mt-3 w-full">
                <select name="c_title" id="c_title" class="border border-gray-400 rounded-md p-1">
                    <option value="" selected disabled>~~Select Title~~</option>
                    <option value="Mr." @php if(session('c_title') == "Mr."){echo 'selected';}else{echo ''; } @endphp>Mr.</option>
                    <option value="Mrs." @php if(session('c_title') == "Mrs."){echo 'selected';}else{echo ''; } @endphp>Mrs.</option>
                    <option value="Miss." @php if(session('c_title') == "Miss."){echo 'selected';}else{echo '';} @endphp>Miss.</option>
                    <option value="Rev." @php if(session('c_title') == "Rev."){echo 'selected';}else{echo '';} @endphp>Rev.</option>
                </select>
                <input type="text" name="c_name" id="c_name" class="border border-gray-400 rounded-md p-1 w-[85%]" value="@php if(session('c_name')){echo session('c_name');}else{echo '';} @endphp">
            </div>
            <div class="mt-2 w-full flex items-center space-x-2">
                <p class="flex text-sm font-bold">
                    <input type="text" name="c_age" class="border border-gray-400 rounded-md p-1" value="@php if(session('c_age')){echo session('c_age');}else{echo '';} @endphp" />
                     &nbsp;years,
                </p>
                <p class="text-sm font-bold"><input type="text" name="c_address" id="c_address" class="border border-gray-400 rounded-md p-1 w-[85%]" value="@php if(session('c_address')){echo session('c_address');}else{echo '';} @endphp"></p>
            </div>
            <div class="w-full mt-[50px]" style="line-height: 2;">
                <textarea name="c_problem" id="c_problem" cols="30" rows="2" class="w-full border border-gray-400 p-1 rounded-md">@php if(session('c_problem')){echo session('c_problem');}else{ echo '';} @endphp</textarea>
            </div>
            <div class="w-full mt-[50px]" style="line-height: 2;">
